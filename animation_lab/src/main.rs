    ...
    event_loop.run(move |event, _, control_flow| {
        match event {
            ...
            Event::MainEventsCleared => {
                ...
                } else if mouse_press[2]
                //right button
                {
                    if now_keys[VirtualKeyCode::Right as usize] {

                        //fig.set_acc(SurfaceType::Muddy, true, 1.0);
                    } else if now_keys[VirtualKeyCode::Left as usize] {
                ...
                //fig.compute_distance();
                //fig.apply_friction(); //LOOK HERE
                /* line(
                    &mut fb2d,
                    WIDTH / 4,
                    WIDTH - WIDTH / 4,
                    y,
                    (255 - (y as u8 % 255), 0, y as u8 % 255, 255),
                );*/

                //LOOK HERE
                /*let back_rect = Rect::new(
                    prev_fig.pos.x as usize,
                    prev_fig.pos.y as usize,
                    prev_fig.size.x as usize,
                    prev_fig.size.y as usize,
                );
                let rect = Rect::new(
                    fig.pos.x as usize,
                    fig.pos.y as usize,
                ...
                rectangle(&mut fb2d, back_rect, prev_fig.color);
                prev_fig = fig.clone();
                prev_fig.color = (128, 64, 64, 255);
                rectangle(&mut fb2d, rect, fig.color);
